...
wmd_model = gensim.models.KeyedVectors.load_word2vec_format('../../day5_prediction/vecs/fastText_model.vec', binary=False)
...
def score_json(method, using_model, lang, model_json_path):
    ...
    filename = model_json_path.stem.replace('.', '_')
    ...
    with open(model_json_path, "r") as f:
        ...
        output_json = {
            "metadata": {
                ...
                "system_prompt": all_day["metadata"][0]["system_prompt"]
            },
            "scores": []
            }
...
def main():
    ...
    response_dir = Path(f"./response/{base_model}")

    for lang in langs:
        json_files = list((response_dir/lang).glob("*.json"))
        for json_file in json_files:
            score_json("Fine-tuning (GPT)", base_model, lang, json_file)
